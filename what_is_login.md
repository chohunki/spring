- 회원가입 및 로그인 가능
  > required 속성을 이용하여 공백을 입력할 수 없게 함
  > 회원가입 후에는 다시 로그인 페이지로 이동
  > 로그인 후에는 게시판 목록으로 이동
  > 로그인 실패 시 alert로 메시지를 띄우고 다시 로그인 화면으로 이동함

- 로그아웃 가능
  > 로그아웃 시 alert로 메시지를 띄우고 로그인 화면으로 이동함

- 게시판 가능
  > 페이징 VO를 별도로 만들어 페이징 구현
  > 한 페이지당 5개의 목록을 보여줌
  > 한 화면에 5개의 페이지 버튼을 보여줌 
  > 세션정보를 이용하여 로그인 한 회원이 작성한 글에만 수정 및 삭제 버튼이 보임
  > 여기서는 스크립트를 활용하여 공백입력이 불가능함

- 마이페이지 가능
  > 회원정보 수정 및 탈퇴 가능
  > 탈퇴 시 비밀번호를 입력받음
  > 비밀번호가 다를 경우 탈퇴가 진행되지 않고 비밀번호 입력페이지로 돌아감
